    ...
    def Player_take_hit(self, Move, Monster):
        ...
        if(random.uniform(0, 1) < Move.accuracy*Monster.accuracy*self.Player_dodge):
            ...
            if(random.uniform(0,1)< Move.crit_chance):
                ...
                print("The attack took you by surprise!")
        ...
        if self.Player_HP<=0:
            self.game_over = True
            self.Player_HP = 0
            print("The room starts to go dark, as you collapse to the ground, blood seeping out through the cracks in your armour. Game Over")
            
    def Player_strike(self, Move, Monster):
        
        if Move.Move_type == "attack":
            print("You try to ", Move.Move_name, " the ", Monster.name)
            Monster.take_hit(self, Move)
            
        if Move.Move_type == "run":
            print("You try to escape")
            if (random.uniform(0,1)<0.25):
                print("You made it away")
                Player.game_over = True #Change this so you run away rather than die
            else:
                print("Damn, that thing is persistent")
        if Move.Move_type == "dodge":
            print("You attempt to evade their next blow")
            if(random.uniform(0,1)<0.75):
                print("You dodge the blow, and manage to whack them in the process")
                Monster.take_hit(self, Move)
                self.justdodged = True # sets bool so you evade next attack
            else:
                print("You weren't quite quick enough")
                self.justdodged = False
...
class Monster(object):
    ...
    def attack(self, Player):
         ...
         if chosen_attack.Move_type == "attack" and self.hp>0 and Player.justdodged == False:
             print("The ", self.name, " attempts to ", chosen_attack.Move_name, " you!")
    ...
    def take_hit(self, Player, Move):
        ...
        if(random.uniform(0, 1) < Move.accuracy*Player.Player_accuracy*self.dodge):
            ...
            if(random.uniform(0,1)< Move.crit_chance):
                points_lost = points_lost*3
                print("The attack took the ", self.name, " by surprise!")
            print("The ", self.name, " takes ", round(points_lost,3), " damage")
        else:
            print("The ", self.name, " dodged the attack!")
        self.hp -= points_lost
        if self.hp<=0:
            self.alive = False
            self.hp =0 
...
PLAYER = Player("Player.png", 100, 0.1, 0.9, 0.9, 0, [(Slash, 10), (Stab, 10), (Punch, 10), (Evade, 10), (Run, 10)])

rat = Monster("rat.png", "rat", 1, 101, 0.1, 0.8, 1, [(Bite, 1), (Bite, 0), (Bite, 0), (Bite, 0)])
strong_rat = Monster("rat.png", "rat", 20, 101, 0.1, 0.8, 1, [(Bite, 1), (Bite, 0), (Bite, 0), (Bite, 0)])


def select_attack(Player):
    print("")
    while(1>0):
        attack = input("What will you do next?: ")
        for mov in Player.Move_list:
            ...
            print("")
            print("Invalid input. Possible actions:")
            i= 1
            for val in Player.Move_list:
                print(i, ".", val[0].Move_name)
                i +=1
            print(" ")
...
def fight(Player, monster):
    ...
    print("A ", monster.name, " is heading straight for you!")
